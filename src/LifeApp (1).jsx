...
// ─────────────────────── CALENDAR (pre-built, ready for OAuth) ───────────────────────
...
function CalendarCard({onNavigate,funList,goals}) {
    ...
    return (
      <div style={{...mkCard({background:"#F0FDF4",border:`1px solid #6EE7B7`})}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
          ...
          <span style={{background:C.amberLight,color:C.amberDark,borderRadius:6,padding:"2px 7px",fontSize:11,fontWeight:700}}>Setting up…</span>
        </div>
        <p style={{fontSize:13,color:C.green,margin:0,opacity:0.8}}>OAuth setup in progress — this card will show your free days and suggest fun plans once connected.</p>
      </div>
    );
  }
  ...
  return (
    <div style={mkCard()}>
      ...
      {loading&&<p style={{fontSize:13,color:C.muted,margin:0}}>Loading calendar…</p>}
      {freeDays.length>0&&(
        <>
          <p style={{fontSize:12,color:C.green,fontWeight:700,margin:"0 0 8px"}}>✅ {freeDays.length} free days in the next 2 weeks</p>
          {suggestedFun.map(f=>(
            <div key={f.id} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 0"}}>
              <span style={{fontSize:16}}>{f.icon}</span>
              <p style={{fontSize:13,margin:0,flex:1}}>{f.title}</p>
              <span style={{fontSize:11,color:C.amber,fontWeight:600}}>Plan it →</span>
            </div>
          ))}
        </>
      )}
